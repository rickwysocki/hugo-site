...
This doesn't fix all problems. I imagine we're probably a year or two away from universities selling all of their teachers' Canvas material to be trained on LLMs, if we're not there already. There are more issues with LMS's that this system can't fix. But it's a good first step, and one I'm appreciating so far.
