# Created 2026-07-11 Sat 08:45
#+title: Emacs: Literate Config
#+author: Rick Wysocki
#+property: header-args :tangle "~/.emacs.d/init.el"
#+export_file_name: ~/Blog/emacs-config.org
It worked
* Preamble
...
- Stay minimal. I generally aim to reduce digital claustrophobia by, whenever possible, choosing the option that leads to fewer files, fewer lines of code, fewer (unnecessary) options, etc.
- Embrace default options. There is more functionality than I could ever need (or learn) built into Emacs. With that in mind, I try to make configuration decisions that err toward using built-in options that ship with Emacs. There are many occasions where I break this convention, but it's a north star.
- Config your own config. Now that I'm a few years into my Emacs journey, with lots of periods where my configuration has gotten totally out of control, I'm very aware of why the documentation suggests not copying others' config files. With some exceptions, I try to write my own configuration blocks. When I have to break this guideliens, I (at least) type them in rather than copy them over. I learn more that way.
...
This block loads my prferred default theme.
...
This block ensures that a time stamp is set when a todo is completed. This is necessary to automatically apply a date for blog posts.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (setq org-capture-templates
        '(("t" "Task" entry
            (file "~/Org/tasks.org")
            (file "~/Org/templates/tpl-todo.txt") :empty-lines-before 1)
        ("j" "Journal entry" entry
           (file+olp+datetree "~/Org/journal.org")
           (file "~/Org/templates/tpl-journal.txt"))
        ...
        ("b" "Book to read" entry       
         (file "~/Org/books.org")
         (file "~/Org/templates/tpl-book.txt") :empty-lines-before 1)))
#+end_src
...
**NOTE: When I first installed ox-hugo, Emacs displayed a number of warnings about depreciated elements. However, when I restarted Emacs, no warnings were displayed. I am currently unsure whether the functionality of =ox-hugo= is compromised.**
#+begin_src emacs-lisp
  ;; Install and configure ox-hugo.
  (use-package ox-hugo
...
**NOTE: I am in the process of rewriting my Hugo workflow/configuration. This will not work, currently.**
...
This block configures extra backends for exporting Org files. Currently, I only require the org backend here.
#+begin_src emacs-lisp
  (require 'ox-org)
#+end_src
